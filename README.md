...
- Profile별 해상도 트랜스코딩 (FHD, HD, D1, CIF)
...
| `/stream` | RTSP 스트림 | 쿼리 파라미터 (`?url=...&profile=...`) |

## /stream 프로토콜

### 연결
```
ws://host:port/stream?url=rtsp://...&profile=HD
```
...
# STUN 서버 (선택사항)
# 온라인 환경: stun.l.google.com:19302
# 오프라인 환경: 비워둠
stun.server.url=
```
...
WebRTC는 H.265를 지원하지 않습니다. 카메라 설정에서 H.264로 변경하세요.
